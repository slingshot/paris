...
**Generic option typing:** `Select`, `Combobox`, and `AccordionSelect` are generic over their option `metadata` type (`<Select<MyMeta> />`), and their `onChange` surfaces typed `metadata`: `Combobox` and `AccordionSelect` receive the full selected `Option<T>` (or `Option<T>[]`), while `Select` receives the selected `id`(s) first and the full `Option<T>`(s) second. `Select` also forwards `name`, `id`, `onBlur`, and `data-*` to the focusable element, so a `react-hook-form` field can be spread onto it directly (`<Select {...field} options={options} />`). `Tabs` is generic over an optional per-tab `id`, surfaced as the second arg of `onTabChange(index, id)`.
