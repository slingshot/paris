...
- f35aea6: Expose overrides for every styled element of `Input` and `Select`.
...
- 950854c: Keep a `Drawer` field visible instead of letting the bottom panel hide it.
...
- b16206c: Make `PhoneInput`, `AccordionSelect`, and `Combobox` work with a bare `{...field}` spread from `react-hook-form`, so call sites no longer need adapter wrappers.
  ...
  **Breaking — `Combobox` `value`, `defaultValue`, and `onChange`:** the combobox is now driven by the option `id` rather than by the option object, so what `onChange` emits can be stored and handed straight back as `value`. `onChange` receives that field value first and the full option second.
  ...
  Because the display is now resolved from `value` against `options`, a value whose option is missing from the list would otherwise render as no selection. Two id-keyed fallbacks cover that: the combobox remembers the last option the user picked, and the new `selectedOption` prop supplies one explicitly — for a prefilled form whose options load asynchronously. Resolution order is `options` match, then the remembered option, then `selectedOption`, then custom text under `allowCustomValue`, then no selection. Both fallbacks apply only when their `id` equals the current value, so a reset or an externally changed value never resurrects a stale display, and `selectedOption` is ignored whenever `options` can resolve the value. Pass `selectedOption` wherever `options` are fetched or filtered.
...
- 0cda3c3: Make `Select` usable with a bare `react-hook-form` field spread.
  ...
  **Breaking — `Select` `onChange`:** it now receives the selected option's `id` first and the full `Option<T>` second (`ids` then `Option<T>[]` for multi-select); both arguments are `null` when the selection is cleared. `value`/`defaultValue` are unchanged (still id-based). Migrate by dropping the `option.id` unwrapping:
  ...
  <Controller
    ...
    control={control}
    render={({ field }) => (
      <Select {...field} options={opts} label="Release type" />
    )}
  />
  ```

- e6542b2: Make `Checkbox` work with a bare form-field spread — `<Checkbox {...field} />` inside a `react-hook-form` `Controller` now needs no `checked={field.value}` adapter.
...
- 017acd3: `Drawer` and `Dialog` now report **why** they closed, and guarantee teardown.
...
- 4d0ca1c: Compile components to ESM instead of shipping unbundled `.tsx`
