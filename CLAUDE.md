...
1. **The ecosystem must be `bun`, not `npm`.** Bun is a first-class Dependabot ecosystem with its own YAML value. Setting `npm` silently half-works: Dependabot detects the package manager as plain npm, finds no `package-lock.json`, and edits `package.json` while leaving `bun.lock` alone — so every PR fails CI at `bun install --frozen-lockfile` with `lockfile had changes, but lockfile is frozen`. Separately, the `bun` ecosystem supports *version* updates but **not** *security* updates, so a Dependabot security alert never produces a fix PR on its own and must be bumped by hand; the weekly version updates are what sweep such fixes up in practice.
...
**Generic Option Components**: `Select`, `Combobox`, and `AccordionSelect` are generic over their option `metadata` and `id` types (`Option<T, Id>`); with literal-id `options`, `value` and the selected option's `id` narrow to that union (`Id` defaults to `string`). All three call `onChange(id, option)`, giving the field value first and the full option (with typed `metadata`) second, so a `react-hook-form` field spreads onto them bare (`<Select {...field} options={options} />` — `name`, `id`, `onBlur`, and `data-*` land on the focusable element). Because `forwardRef` erases generics, these components cast their `forwardRef` export back to a generic function type so `<Select<MyMeta> />` keeps working — follow that pattern when adding a generic + ref-forwarding component. `Tabs` is generic over an optional per-tab `id` passed to `onTabChange`.

**Form Focus & Error State**: form inputs forward a `ref` to a focusable element for `react-hook-form` `setFocus` (the ref target must stay mounted and focusable — e.g. `Select`'s radio/card/segmented kinds forward to the option holding the group's roving tab stop, and `Combobox` falls back to its container when the input unmounts). Components without a DOM input (`MarkdownEditor`) expose an imperative `{ focus() }` handle via `useImperativeHandle` instead. Field-like components take `status?: 'default' | 'error'` and render error state via a `data-status="error"` attribute styled in SCSS.

**Field-Spread Compatibility**: a bare `{...field}` spread from `react-hook-form` must work without a call-site adapter — the component takes `name` and `onBlur` and puts them on the focusable element, and treats `undefined` like an empty value rather than switching to uncontrolled mode. `PhoneInput` detects controlled mode by the presence of the `value` prop (`'value' in props`), not by `value !== undefined`, so a field that resets to `undefined` still clears. The value a component emits must be the same shape it accepts back as `value`, so option components are keyed on the option `id`, not the option object. Where a component merges a consumer handler with built-in behaviour, run the consumer's first and let `event.defaultPrevented` suppress the built-in.
